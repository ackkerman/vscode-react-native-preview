...
- Added ESLint configuration and npm scripts for lint/test along with extension tests powered by @vscode/test-electron and xvfb-run to support headless environments.
- Patched the vscode-test download validation to tolerate missing content-length headers while preserving checksum validation so extension tests can download VS Code builds reliably.
